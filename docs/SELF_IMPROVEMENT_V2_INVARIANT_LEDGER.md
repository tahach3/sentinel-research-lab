...
| INV-WF-03 | Workflow **node-set closure**: every declared node must be on the allow-list (`ALLOWED_NODE_SPECS`); extras (even unconnected) deny-by-default — **rigidity is the control**; making the node set open reopens NP-3 | Codex NP-3; parse-strict repair | established |
| INV-TOPO-01 | Option A: `srl-worker` shares n8n’s network namespace (`network_mode: service:n8n`), binds `127.0.0.1:8765` unpublished, and is attested by a **host** attestor on `127.0.0.1:8764`. Identity is the closed 21-field schema; Consume→Attest→Authorize→Agent; distinct Implementer/Reviewer/Execute attest nodes; single-use topology-binding token; host-wide `docker ps -a` closed container set; sanitized `DOCKER_*`; no docker.sock / privileged / host net. Execute uses an independent `/tmp/srl-exec/<hex32>` clone (RO reviewed source including `.git`); export is `refs/srl/export/<id>` + bundle/diff with ACK-then-cleanup; human promotion does not merge | Option A accepted design; `topology_attest` / `git_worker` / `candidate_export`; F1–F13 tests | established (code); live Docker Desktop reassert residual |
