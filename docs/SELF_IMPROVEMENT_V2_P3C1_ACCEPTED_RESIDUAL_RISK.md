...
| V-INSTALL re-root | Re-run `install_launcher` from a compromised tree to mint a matching digest | Same local-code-execution adversary as D2-BYPASS. **Named properties of this boundary (not silent oversights):** `MECHANICAL_AUTH_BINDING=ABSENT` — reinstall is not mechanically tied to the authorization ritual; `LIVE_AUTH_REVOCATION=ABSENT` — a successful reinstall that changes accepted HEAD/digest does not revoke a live authorization line. These are **in scope of this residual record** (amended deliberately after Codex wide review), not a separate unscoped acceptance |

**Not accepted as residual / must stay closed in code:** incomplete pin vs closure (N1 + launcher pin), N11 HEAD drift tolerance, ambient `GIT_*` rebinding, dirty/drifted reinstall accidents, `repository_root` divergence (R4), validator multi-dispatch / port / disabled / **non-`main` attachment** gaps (N2–N7, R1), transport omit/burn bugs, **parse-strictly-or-reject** on every workflow JSON walk including **link.type / link.index**, top-level/object shape, node-member shape, and **pinned** `workflow_validator` (Codex NP-2/3/4 + link-mutation class).
