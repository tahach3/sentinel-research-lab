...
def bridge_env(tmp_path: Path):
    ...
    boot.write_text(
        ...
        "NID = 'b' * 64\n"
        "DIGEST = 'sha256:' + 'c' * 64\n"
        "SEALED = re.compile(r'^([a-f0-9]{64}):(/tmp/srl-exec/[a-f0-9]{32}/export/[a-f0-9]{8}/([^/\\\\:]+))$')\n"
        "fs = {}\n"
        "repos = {}\n"
        "def _inspect(cid, started='t0', image=DIGEST, netns='net-1'):\n"
        "    return {'Id': cid, 'State': {'Status': 'running', 'StartedAt': started},\n"
        "            'Image': image, 'NetworkSettings': {'SandboxKey': netns},\n"
        ...
        "        outbound, inbound = SEALED.fullmatch(src), SEALED.fullmatch(dest)\n"
        "        if outbound and inbound is None:\n"
        "            Path(dest).parent.mkdir(parents=True, exist_ok=True)\n"
        "            Path(dest).write_bytes(fs[src])\n"
        "            return 0, '', ''\n"
        "        if inbound and outbound is None:\n"
        "            fs[dest] = Path(src).read_bytes()\n"
        "            return 0, '', ''\n"
        "        return 1, '', 'unclassified docker cp'\n"
        "    if len(argv) >= 4 and argv[1] == 'exec':\n"
        "        i = 2\n"
        "        while i + 1 < len(argv) and argv[i] == '--env':\n"
        "            i += 2\n"
        "        cid = argv[i]\n"
        "        cmd = argv[i+1:]\n"
        "        if cmd[:2] == ['mkdir', '-p']:\n"
        ...
        "            return 0, fs[f'{cid}:{cmd[1]}'].decode('utf-8'), ''\n"
        ...
        "            bf = tmp / 'candidate.bundle'\n"
        "            bf.write_bytes(fs[f'{cid}:{bundle}'])\n"
        "            dest = tmp / 'repo'\n"
        "            proc = subprocess.run(['git', 'clone', '--branch', 'srl-candidate', str(bf), str(dest)], capture_output=True, text=True)\n"
        "            if proc.returncode == 0:\n"
        "                repos[repo] = dest\n"
        "            return proc.returncode, proc.stdout or '', proc.stderr or ''\n"
        "        if len(cmd) >= 4 and cmd[0] == 'git' and cmd[1] == '-C':\n"
        "            proc = subprocess.run(['git', '-C', str(repos[cmd[2]]), *cmd[3:]], capture_output=True, text=True)\n"
        "            return proc.returncode, proc.stdout or '', proc.stderr or ''\n"
        "        return 1, '', 'unclassified docker exec'\n"
        "    return 1, '', 'unexpected docker argv'\n"
        "def _copy_fn(argv):\n"
        "    code, _out, err = _runner(argv)\n"
        "    if code != 0:\n"
        "        raise WorkerError('FAILED_FROZEN', err or 'docker cp failed', state='FAILED_FROZEN')\n"
        "config = RuntimeConfig(\n"
        f"    repository_root=Path({root!r}),\n"
        f"    state_db=Path({str(state_db.resolve())!r}),\n"
        f"    worker_token={TOKEN!r},\n"
        "    worker_host='127.0.0.1',\n"
        "    worker_port=0,\n"
        "    rev25_deps=Rev25RuntimeDeps(\n"
        "        inspect_worker=lambda cid: _inspect(cid),\n"
        "        inspect_n8n=lambda cid: _inspect(cid, 'n8n-t0', DIGEST, 'net-1'),\n"
